        ...
        for col in range(BOARD_COLS):
            if board[row][col] == 1:
                ...
                pygame.draw.line(screen, color, 
                                 ...
                                 width=CROSS_WIDTH)
                pygame.draw.line(screen, color, 
                                 start_pos=(col * SQUARE_SIZE + 3 * SQUARE_SIZE // 4, row * SQUARE_SIZE + SQUARE_SIZE // 4), 
                                 end_pos=(col * SQUARE_SIZE + SQUARE_SIZE // 4, row * SQUARE_SIZE + 3 * SQUARE_SIZE // 4), 
                                 width=CROSS_WIDTH)
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if minimax_board[row][col] == 0:
                    ...
                    score = minimax(minimax_board=minimax_board, depth=depth+1, is_maximizing=False)
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if minimax_board[row][col] == 0:
                    ...
                    score = minimax(minimax_board=minimax_board, depth=depth+1, is_maximizing=True)
                    minimax_board[row][col] = 0
...
def best_move():
    if game_over:  # Check if the game is already over
        return False
    ...
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLS):
            if board[row][col] == 0:
                board[row][col] = 2
                score = minimax(minimax_board=board, depth=0, is_maximizing=False)
                board[row][col] = 0
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            if avaliable_square(mouseX, mouseY):
                mark_square(row=mouseY, col=mouseX, player=player)
                ...
                # Handle draw check only if no winner and board is full
                if not game_over:
                    if is_board_full():
                        game_over = True
